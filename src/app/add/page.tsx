...
export const allServices = [
  {
    id: 'custom',
    name: '직접 입력하기',
    iconUrl: <Plus className="mb-2" aria-label="구독 내용 직접 입력하기" size={20} strokeWidth={3} />,
    href: 'add/detail',
    isCustom: true,
  },
  ...KNOWN_SERVICES.map((service) => ({
    id: service.id,
    name: service.name,
    iconUrl: <Image src={service.iconUrl} className="mb-2" alt={service.name} width={20} height={20} />,
    href: `add/detail`,
    isCustom: false,
  })),
];
...
export default function Page() {
  ...
  const handleCardClick = (service: { id: string; name: string; href: string; iconUrl: JSX.Element; isCustom: boolean }) => {
    setSelectedService({
      id: service.id,
